  ...
  <div class="card">
      ...
      <tbody>
        <c:forEach items="${list}" var="h">
          <tr>
            <td>${h.postnum}</td>
            <td>${h.id}</td>
            <td><a href="fleadetail?postnum=${h.postnum}">${h.posttitle}</a></td>
            <td>${h.postcnt}</td>
          </tr>
        </c:forEach>
      </tbody>
    ...
    <div class="paging">
      <c:if test="${paging.startPage != 1}">
        <a href="fleaout?nowPage=${paging.startPage-1}&cntPerPage=${paging.cntPerPage}">이전</a>
      </c:if>
      ...
      <c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="p">
        <c:choose>
          ...
          <c:otherwise>
            <a href="fleaout?nowPage=${p}&cntPerPage=${paging.cntPerPage}">${p}</a>
          </c:otherwise>
        </c:choose>
      </c:forEach>

      <c:if test="${paging.endPage != paging.lastPage}">
        <a href="fleaout?nowPage=${paging.endPage+1}&cntPerPage=${paging.cntPerPage}">다음</a>
      </c:if>
    </div>
  </div>
