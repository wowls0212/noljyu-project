...
$(function(){
    $("#comdelete").click(function(){
        if(confirm("정말로 삭제하시겠습니까?")){
            $.post("comdelete", { comnum: $('#comnum').val() }, function(res){
                if(res==='success'){
                    alert('삭제가 완료되었습니다');
                    location.replace('comout');
                } else {
...
function reviewRealUpdate(idx){
    $.post('comreviewupdate', { reviewnum: $('#reviewnum'+idx).val(), review: $('#reveiwUpdate-content'+idx).val() }, function(data){
        if(data==='1') location.replace('comdetail?postnum=${dto.postnum}');
    });
}

function comreupdate(idx){
    $('#comreview2'+idx).hide();
    $('#comreview3'+idx).show();
    $('#comreupdate-btn'+idx).hide();
}

function comreviewdelete(idx){
    if(confirm('정말로 삭제하시겠습니까?')){
        $.post('comreviewdelete',{ reviewnum:$('#reviewnum'+idx).val() },function(res){
            if(res==='success') location.replace('comdetail?postnum=${dto.postnum}');
            else alert('정상적으로 처리되지 않았습니다');
        });
    }
}

function comrere(idx){
    $('#comrere2'+idx).show();
    $('#comrere-btn'+idx).hide();
}

function rereRealUpdate(idx){
    $.post('comreresave', {
        reviewnum: $('#reviewnum'+idx).val(),
        review: $('#comrere-content'+idx).val(),
        id: $('#id'+idx).val()
    }, function(d){
        if(d==='1') location.replace('comdetail?postnum=${dto.postnum}');
    });
}
</script>
</head>
...
<div class="container">
  ...
  <!-- 게시글 상세 -->
  <div class="card">
    <input type="hidden" id="comnum" value="${dto.postnum}" />
    ...
    <!-- 추천 버튼 -->
    <div class="goodbtn">
      <form action="good3" style="display:inline;">
    ...
    <c:if test="${loginstate==true }">
      <c:if test="${id==dto.id or id=='admin'}">
        <button class="btn btn-edit" onclick="location.href='comupdate?comnum=${dto.postnum}'">수정</button>
        <button class="btn btn-delete" id="comdelete">삭제</button>
      </c:if>
    ...
    <table width="800px">
      <tbody>
        <c:forEach items="${list}" var="rr" varStatus="status">
          <tr height="80px">
            ...
            <c:if test="${loginstate==true}">
              <c:if test="${id==rr.id or id=='admin'}">
                <button id="comreupdate-btn${status.index}" onclick="comreupdate(${status.index});return false;">수정</button>
                <button onclick="comreviewdelete(${status.index})">삭제</button>
              </c:if>
              <button id="comrere-btn${status.index}" onclick="comrere(${status.index});return false;">댓글</button>
          ...
          <tr id="comrere2${status.index}" style="display:none;">
            <td colspan="2">
              ...
              <textarea id="comrere-content${status.index}" rows="2" cols="100" placeholder="댓글을 입력해 주세요"></textarea>
        ...
        <c:if test="${empty list}">
          <tr>
            <td colspan="2" style="text-align:center; padding:20px 0; color:#666;">
              댓글이 없습니다.
            </td>
          </tr>
        </c:if>
        ...
        <tr style="border:none;">
          <td colspan="2" style="text-align:center;">
            <c:if test="${paging.startPage>1}">
              <a href="comdetail?postnum=${dto.postnum}&nowPage=${paging.startPage-1}&cntPerPage=${paging.cntPerPage}">이전</a>
            </c:if>
            ...
            <c:if test="${paging.endPage<paging.lastPage}">
              <a href="comdetail?postnum=${dto.postnum}&nowPage=${paging.endPage+1}&cntPerPage=${paging.cntPerPage}">다음</a>
            </c:if>
          </td>
        </tr>
      </tbody>
