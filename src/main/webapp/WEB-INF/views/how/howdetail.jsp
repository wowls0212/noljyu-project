...
$(function(){
    $('#howdelete').click(function(){
        if(confirm('정말로 삭제하시겠습니까?')){
            $.post('howdelete',{ hownum: $('#hownum').val() },function(res){
                if(res==='success'){ alert('삭제가 완료되었습니다'); location.replace('howout'); }
                else { alert('정상적으로 처리되지 않았습니다'); location.href='main'; }
            });
        }
    });
});
function reviewRealUpdate(idx){
    $.post('howreviewupdate',{ reviewnum: $('#reviewnum'+idx).val(), review: $('#reveiwUpdate-content'+idx).val() },function(data){ if(data==='1') location.replace('howdetail?postnum=${dto.postnum}'); });
}
function howreupdate(idx){ $('#howreview2'+idx).hide(); $('#howreview3'+idx).show(); $('#howreupdate-btn'+idx).hide(); }
function howreviewdelete(idx){ if(confirm('정말로 삭제하시겠습니까?')){ $.post('howreviewdelete',{ reviewnum: $('#reviewnum'+idx).val() },function(res){ if(res==='success') location.replace('howdetail?postnum=${dto.postnum}'); else alert('정상적으로 처리되지 않았습니다'); }); }}
function howrere(idx){ $('#howrere2'+idx).show(); $('#howrere-btn'+idx).hide(); }
function rereRealUpdate(idx){
    $.post('howreresave',{ reviewnum: $('#reviewnum'+idx).val(), review: $('#howrere-content'+idx).val(), id: $('#id'+idx).val() },function(d){ if(d==='1') location.replace('howdetail?postnum=${dto.postnum}'); });
}
</script>
</head>
<body>
...
<div class="container">
  <!-- 게시글 상세 -->
  <div class="card">
    <input type="hidden" id="hownum" value="${dto.postnum}" />
    ...
    <c:if test="${loginstate==true }">
      <c:if test="${id==dto.id or id=='admin'}">
        <button class="btn btn-edit" onclick="location.href='howupdate?hownum=${dto.postnum}'">수정</button>
        <button class="btn btn-delete" id="howdelete">삭제</button>
      </c:if>
    ...
    <table width="800px">
      <tbody>
        <c:forEach items="${list}" var="rr" varStatus="status">
          <tr height="80px">
            <th>
              <c:forEach var="i" begin="0" end="${rr.postindent}">
                <c:choose>
                  ...
                  <c:when test="${i==rr.postindent}"><img src="./image/foot.jpg" height="20" style="vertical-align:middle;"></c:when>
                </c:choose>
              </c:forEach>
              ${rr.id}
            ...
            <c:if test="${loginstate==true }">
              ...
              <button id="howreupdate-btn${status.index}" onclick="howreupdate(${status.index});return false;">수정</button><button onclick="howreviewdelete(${status.index})">삭제</button>
              </c:if>
              <button id="howrere-btn${status.index}" onclick="howrere(${status.index});return false;">댓글</button>
          ...
          <tr id="howreview3${status.index}" style="display:none;"><td colspan="2">
          ...
          <a href="#" class="btn btn-delete">취소</a></td></tr>
          <tr id="howrere2${status.index}" style="display:none;">
          <td colspan="2">
          <input type="hidden" id="id${status.index}" value="${id}">
          <textarea id="howrere-content${status.index}" rows="2" cols="100" placeholder="댓글을 입력해 주세요"></textarea>
        ...
        <tr style="border:none;">
        	<td colspan="2" style="text-align:center;">
        		<c:if test="${paging.startPage>1}">
        			<a href="howdetail?postnum=${dto.postnum}&nowPage=${paging.startPage-1}&cntPerPage=${paging.cntPerPage}">이전</a>
        		</c:if>
        ...
        <c:if test="${paging.endPage<paging.lastPage}">
        	<a href="howdetail?postnum=${dto.postnom}&nowPage=${paging.endPage+1}&cntPerPage=${paging.cntPerPage}">다음</a>
        </c:if>
        </td>
        </tr>
      </tbody>
  ...
  <div class="card">
    <form action="howreviewsave">
      ...
      <textarea rows="5" name="review" placeholder="댓글을 작성해주세요">
      </textarea><div class="button-group" style="margin-top:10px;">
      <c:if test="${loginstate==true }">
