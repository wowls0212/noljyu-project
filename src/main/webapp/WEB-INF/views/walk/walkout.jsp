...
<body>
  ...
  <div class="walkpage">
		<c:if test="${paging.startPage!=1 }">
		   <a href="walkout?nowPage=${paging.startPage-1 }&cntPerPage=${paging.cntPerPage}"></a>
		</c:if>   
		...
		<c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="p"> 
		   <c:choose>
		      ...
		      <c:when test="${p != paging.nowPage}">
		         <a href="walkout?nowPage=${p}&cntPerPage=${paging.cntPerPage}">${p}</a>
		      </c:when>   
		   </c:choose>
		</c:forEach>      
		
		<c:if test="${paging.endPage != paging.lastPage}">
			<a href="walkout?nowPage=${paging.endPage+1}&cntPerPage=${paging.cntPerPage }">  </a>
		</c:if>    
  </div>
  ...
  <!-- 검색 폼 -->
    <div class="search-form">
        <form action="walksearch" method="post">
            ...
            <input type="button" onclick="location.href='walkwrite'" value="산책코스 추천">
        </form>
    </div>
</body>
</html>